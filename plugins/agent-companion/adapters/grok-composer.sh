...
# The model id is resolved from `grok models` (no hardcoded version); if no
# grok-composer model is available the agent is SKIPPED.
set -uo pipefail
...
case "$cmd" in
  probe)
    ...
    [ -n "$(resolve_model grok-composer)" ] || exit 64   # no composer model → skip (not fail)
    exit 0;;
  run)
