# Module-reactive stores — full-file etalon (SSR)

Variant: projectType=ssr

Three representative shapes: an **entity store** with persistence (session auth — SSR arm registers a hydration callback instead of reading storage at module-init; see `store.md` for the CSR arm), a **widget view-state** singleton (sidebar open/closed + active view, same `reactive()`/setter/`useX()` skeleton as the entity store), and a **feature state** composable (login action, per-mount, derived entirely via `computed` — nothing module-level).

Mutually exclusive with `store.md`, chosen by `projectType` (the `vue-work` skill's step 0) — reproduce only one.
...
SSR arm — the server has no `localStorage`, so the state stays at its default
value until a hydration callback (the `hydration` skill) runs once on the
client after the root `<Suspense>` resolves:
...
// Guarded: on the server this module is shared across every concurrent request in
// the process, so a setter call must never mutate the singleton there — it would
// leak one visitor's authenticated flag into another visitor's render.
