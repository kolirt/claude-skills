# project-init (Vue) — project scaffold etalon

Variant: projectType=ssr

Scope: root config/scripts only. Bootstrap entry files/`createApp` are owned by `core/references/bootstrap-ssr.md`; robots.txt conventions/policy by the `robots` skill. `vite.config.ts` is reproduced in full, not truncated. SSR only — see sibling `project-scaffold.csr.md` for CSR/SPA; do not hand-reduce this file's scripts/deps yourself. FSD (`{app}` = `01-app`; non-FSD resolves via `core/architectures/<a>.md`). `build:dev`/`build:prod` show the desired mode-aware shape the `project-init` invariant requires — the source project's real script is still mode-less.
...
```json
{
  ...
  "type": "module",
  "scripts": {
    "build:dev": "run-s type-check lint build:dev:bundles build:server-bootstrap",
    "build:prod": "run-s type-check lint build:prod:bundles build:server-bootstrap",
    "build:dev:bundles": "run-p build:dev:client build:dev:server",
    "build:prod:bundles": "run-p build:prod:client build:prod:server",
    "build:dev:client": "vite build --outDir dist/client --ssrManifest --mode development",
    "build:dev:server": "vite build --outDir dist/server --ssr {app}/entryServer.ts --mode development",
    "build:prod:client": "vite build --outDir dist/client --ssrManifest --mode production",
    "build:prod:server": "vite build --outDir dist/server --ssr {app}/entryServer.ts --mode production",
    "build:server-bootstrap": "tsc --project server/tsconfig.json",
    "build-only:dev": "run-s build:dev:bundles build:server-bootstrap",
    "build-only:prod": "run-s build:prod:bundles build:server-bootstrap",
    ...
    "preview": "NODE_ENV=production node dist/server-bootstrap/index.js",
