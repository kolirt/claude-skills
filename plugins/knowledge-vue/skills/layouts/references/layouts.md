...
`vue-work` step 0): this file for SSR, `layouts.csr.md` for CSR — never both.
...
SSR arm: `runHydrations` (the `hydration` skill, SSR-only) reconciles
browser-only state against server-rendered markup once the tree resolves.
`useTitleTemplate('<your-app-name>')` is a non-token placeholder
(`core/placement.md`) — substitute the real app name, never reproduce
`<your-app-name>` literally:
  ...
  <div id="portal" class="fixed top-0 left-0 flex h-screen w-screen flex-col">
    <ErrorBoundary ref="boundaryRef">
      ...
      <component :is="route.meta?.layout?.component ?? 'div'">
        <Suspense @resolve="runHydrations">
          <RouterView />
        </Suspense>
      </component>
    </ErrorBoundary>
